...
async function main() {
  ...
  // Hesap Planı - Tekdüzen Hesap Planı
  const hesaplar = [
    ...
    { code: "770", name: "Genel Yönetim Giderleri Yansıtma", type: "GIDER" },
  ];
  ...
  for (const c of calisanlar) {
    await prisma.employee.create({
      data: {
        ...
        departmentId: departments[c.department],
        status: "AKTIF",
      },
    });
  }
  ...
  // Depolar
  const depo1 = await prisma.warehouse.create({
    data: { name: "Ana Depo", address: "İstanbul, Tuzla OSB" },
  });
  const depo2 = await prisma.warehouse.create({
    data: { name: "Ankara Depo", address: "Ankara, OSTİM" },
  });
  ...
  for (const u of urunler) {
    ...
    await prisma.stockLevel.create({
      data: { productId: p.id, warehouseId: depo1.id, quantity: Math.floor(Math.random() * 100) + 5 },
    });
    await prisma.stockLevel.create({
      data: { productId: p.id, warehouseId: depo2.id, quantity: Math.floor(Math.random() * 50) + 2 },
    });
  }
  ...
  await prisma.opportunity.create({
    data: {
      title: "ABC Teknoloji - Yıllık BT Altyapı Yenileme",
      value: 500000,
      stage: "TEKLIF",
      probability: 60,
  ...
  await prisma.opportunity.create({
    data: {
      title: "Mega İnşaat - Şantiye Ekipman Tedariği",
      value: 250000,
      stage: "MUZAKERE",
      probability: 75,
  ...
  await prisma.opportunity.create({
    data: {
      title: "Star Lojistik - Depo Otomasyon Projesi",
      value: 180000,
      stage: "ADAY",
      probability: 20,
    },
